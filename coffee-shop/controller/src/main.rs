...
fn parse_args() -> Command {
    Command::new("dephy-gacha-controller-node")
        ...
        .version(coffee_shop_controller::VERSION)
        .arg(
            Arg::new("NOSTR_RELAY")
                .long("nostr-relay")
                .num_args(1)
...
async fn controller(args: &ArgMatches) {
    ...
    let client = coffee_shop_controller::RelayClient::new(nostr_relay, &keys, SESSION, 4096)
        .await
        .expect("Failed to connect to relay");

    let message_handler = coffee_shop_controller::node::MessageHandler::new(
        client,
        solana_rpc_url,
        solana_keypair_path
            .to_str()
